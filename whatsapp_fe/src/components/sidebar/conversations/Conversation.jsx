...
function Conversation({ convo, socket, online, typing }) {
  ...
  return (
    <li
      ...
      className={`list-none h-[72px] w-full dark:bg-dark_bg_1 hover:${convo._id !== activeConversation._id ? "dark:bg-dark_bg_3" : ""} cursor-pointer dark:text-dark_text_1 px-[10px] ${convo._id === activeConversation._id ? "dark:bg-dark_hover_1" : ""}`}
    >
              ...
              <div className="flex items-center gap-x-1 dark:text-dark_text_2">
                <div className="flex-1 items-center gap-x-1 dark:text-dark_text_2">
                  {typing === convo._id ? (
                    ...
                    <p>{convo.latestMessage?.message?.length > 25 ? `${convo.latestMessage.message.substring(0, 25)}...` : convo.latestMessage?.message || "No messages yet"}</p>
                  )}
                </div>
              </div>
            </div>
